Main = {
  ...
  setupClickPay: async () => {
    $("#approve-ox").on("click", async (e) => {
      ...
      Main.buttonLoadingHelper(e, "approving...", async () => {
        await Main.ox
          .approve(Main.omegaIdo.address, amount, { from: Main.account })
    ...
    $("#buy-omega").on("click", async (e) => {
      Main.buttonLoadingHelper(e, "buying OM...", async () => {
        ...
        await Main.omegaIdo
          .sellOmegaToken(Main.account, omegaAmount, { from: Main.account })
          .once("transactionHash", async (txHash) => {
  ...
  buttonLoadingHelper: async (event, loadingText, callback) => {
    ...
    try {
      ...
      alert(
        "There was a problem. Please check the values you inputed and try again."
      );
      window.location.reload();
    }
    window.location.reload();
